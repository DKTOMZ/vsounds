...
export const Order = () => {
    ...
    useEffect(()=>{
        if (orders.length > 0) {
            const filter = orders.filter((order)=>order.customerId===query);
            if (filter.length > 0) { 
                setOrder(filter[0]);
                setOrderDate(new Date(filter[0].timestamp));
                setUserInfo(
                    {
                        address:filter[0].shipping['address'],
                        name:filter[0].shipping.name,
                        email: filter[0].shipping.email,
                        phone: filter[0].shipping.phone
                    }
                );
                setLoading(false);
            }
        ...
        <>
            {!loading ?
                <div id="order">
                    ...
                    <h3>ORDER {query}</h3>
                    <div style={{color:'#cccaca'}} className="date">Last update: 
                    ...
                    {order.products.map((product,index)=>{
                        return <div style={{marginBottom:'10px', padding: '10px'}} key={index} className="order-card">
                            <div className="desc2">
                    ...
                    <div style={{padding:'10px',marginTop:'10px',marginBottom:'10px',backgroundColor:'#2c3640'}} className="payment">
                        <p style={{fontWeight:'bold'}}>CUSTOMER INFO</p>
                        <p style={{fontSize:'14px',marginBottom:'0'}}>{userInfo.name}</p>
                        <p style={{fontSize:'14px',marginBottom:'0'}}>{userInfo.email}</p>
                        <p style={{fontSize:'14px',marginBottom:'0'}}>{userInfo.address.line1}</p>
                        <p style={{fontSize:'14px',marginBottom:'0'}}>{userInfo.address.city}</p>
                        <p style={{fontSize:'14px',marginBottom:'0'}}>{userInfo.address.postal_code}</p>
                        <p style={{fontSize:'14px',marginBottom:'0'}}>{userInfo.address.country}</p>
                        <p style={{fontSize:'14px',marginBottom:'0'}}>{userInfo.phone}</p>
                    </div>
                    <div style={{padding:'10px',marginTop:'10px',marginBottom:'30px',backgroundColor:'#2c3640'}} className="payment">
                        ...
                        <div style={{fontSize:'14px'}} className="shipping">Products: {order.products.length}</div>
